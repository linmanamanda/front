<template>
  <div class="report-reply-view">
    ...
    <el-table
      :data="data"
      v-loading="loading"
      element-loading-text="正在全力加载评论信息举报信息中"
...
<script>
  ...
  export default {
    ...
    created() {
      this.getReplyReports()
    },
    watch: {
      '$route': 'getReplyReports',
    },
    methods: {
      getReplyReports() {
        api.getReplyReports(this.query)
          .then((response) => {
      ...
      initBan(row) {
        ...
        this.banForm.replyId = row.replyId
      },
      confirmBan() {
        api.banReply(this.banForm) 
          .then((response) => {
            let code = response.code
            if (code === 0) {
      ...
      confirmDeletion() {
        api.deleteReplyReport(this.deletionForm)
          .then((response) => {
            let code = response.code
            if (code === 0) {
